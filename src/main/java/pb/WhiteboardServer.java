...
public class WhiteboardServer {
	...
	private static Set<Endpoint> liveClients = new HashSet<Endpoint>();
	...
	public static void main( String[] args ) throws IOException, InterruptedException
    {
        ...
        serverManager.on(ServerManager.sessionStarted, (eventArgs)->{
        	...
        	liveClients.add(endpoint);
        	for(int i=0;i<sharingBoards.size();i++) {
        	...
        	endpoint.on(shareBoard, (eventArgs2)->{
        		...
        		synchronized(liveClients) {
					Iterator iter = liveClients.iterator();
					while (iter.hasNext()) {
						Endpoint x = (Endpoint)iter.next();
						x.emit(sharingBoard, board);
					}
				}
        		...
        		synchronized(liveClients) {
					...
					while (iter.hasNext()) {
						Endpoint x = (Endpoint)iter.next();
						x.emit(unsharingBoard, board);
					}
        		}
        	});
        }).on(ServerManager.sessionStopped, (eventArgs)->{
        	Endpoint server = (Endpoint)eventArgs[0];
